...
export const Pong: React.FC<Props> = (props) => {
  //listen subscription of game id
...
const CanvasPong: React.FC<{}> = () => {
  ...
  function keyUpHandler(e: KeyboardEvent) {
    ...
    if (e.key == 'o') {
      controls.O_Key = false
    }
    if (e.key == 'l') {
      controls.L_Key = false
    }
    if (e.key == 'ArrowUp') {
  ...
  function updateElements(delta: number) {
    ...
    if (
      ball.vPos >= racket1.vPos &&
      ball.vPos <= racket1.vPos + racket1.height &&
      ball.hPos > racket1.hPos + racket1.width &&
    ...
    if (
      ball.vPos >= racket2.vPos &&
      ball.vPos <= racket2.vPos + racket2.height &&
      ball.hPos < racket2.hPos &&
    ...
    if (
      ...
      (controls.S_Key || controls.Down_Key)
    ) {
      racket1.vPos += racket1.velocity
